...
| 地図 | Leaflet + OpenStreetMap（API キー不要） |
| データ層 | In-memory mock（提案用デモ）/ 本番は Supabase PostgreSQL + Prisma 切替可能 |
| データ更新 | Server Actions（Form action 直結、Client Component 不要） |
| 認証 | デモモード（Cookie でユーザー切替）/ 本番は NextAuth Google OAuth 切替可能 |
| ホスティング | Vercel（Hobby 無料枠でデモ運用可能） |

---

## 法令・運用面の配慮

- **位置情報の取り扱い**: 訪問開始時の現在地のみ取得。移動中の連続トラッキングは行わない
- **個人情報**: 取引先・連絡先・商談メモは社内利用前提、外部 API への送信は行わない（音声文字起こし機能を有効化する場合は Phase 2 で個別合意）
- **データ保持**: デモモードでは永続化なし。本番モードでは Supabase の暗号化ストレージに保管、定期バックアップ
