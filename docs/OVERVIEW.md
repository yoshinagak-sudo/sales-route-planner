...
当日の訪問予定を時刻順カードで表示。各カードに会社名・訪問予定時刻・状態バッジ（予定 / 訪問中 / 完了）・用件・カテゴリが並び、上部には「完了 / 訪問中 / 予定」の件数サマリーと「予定総距離・移動目安」を表示します。各カード右端のナビアイコンをタップすると Google マップで道案内が起動し、移動の判断と訪問記録への流れを途切れさせません。
